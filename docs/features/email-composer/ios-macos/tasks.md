...
- **Deliverables**:
  - [x] `ComposerView.swift` — presented as sheet on iOS, window on macOS (uses @State, @Environment, .task)
  - [x] Support 4 composition modes: new, reply, reply-all, forward (FR-COMP-01 modes table)
  ...
  - [x] Formatting toolbar (bold, italic, link buttons) that inserts Markdown syntax — users don't type raw Markdown (FR-COMP-01)
...
- **Deliverables**:
  ...
  - [x] `ContactCacheEntry.swift` — feature-local SwiftData `@Model` entity (spec Section 5)
  - [x] `QueryContactsUseCase` — domain use case for querying contact cache (spec Section 6)
  ...
  - [x] Multi-account deduplication: merge suggestions across accounts, keep most recent displayName and highest frequency (FR-COMP-04)
  - [x] Cascade delete contact cache entries when account is removed (FR-COMP-04)
  - [x] Contact cache population during email sync: extract From/To/CC and upsert into ContactCacheEntry (FR-COMP-04)
  - [x] Email address validation (reject malformed addresses with visual indication) (FR-COMP-01)
  ...
  - [x] Unit tests for `QueryContactsUseCase` with 1,000+ cached entries
  - [x] Unit tests for email address validation
  - [x] SwiftUI previews for empty, typing, suggestions visible, tokens added
...
- **Deliverables**:
  - [x] `SaveDraftUseCase` — domain use case for persisting drafts to SwiftData + IMAP (spec Section 6)
  - [x] `DeleteDraftUseCase` — domain use case for deleting drafts on send/discard (spec Section 6)
  ...
  - [x] Send state machine: none (draft) → queued (undo window) → sending → sent/failed (per Foundation Section 5.5 mapping in FR-COMP-02)
  ...
  - [x] Unit tests for send state machine (all transitions)
  - [x] Unit tests for draft auto-save lifecycle
  - [x] Unit tests for undo-send edge cases (termination, background, offline)
...
- **Deliverables**:
  ...
  - [x] File picker: iOS `UIDocumentPickerViewController`, macOS `NSOpenPanel` (FR-COMP-01)
  ...
  - [x] Total size warning at 25 MB; prevent send if over limit (FR-COMP-01)
  - [x] Forward mode: list original attachments, allow removal, download undownloaded before send (FR-COMP-01)
  ...
  - [x] Unit tests for attachment size validation
  - [x] Unit tests for forward attachment handling
  - [x] SwiftUI previews for: no attachments, with attachments, size warning, smart reply chips

### Implementation Notes (2026-02-08)

- V1 editor behavior intentionally uses raw Markdown in `TextEditor`; safe HTML conversion remains in send pipeline utilities.
- Inline image insertion remains deferred by product decision for this PR.
- Attachment photo-library and camera pickers, macOS drag-and-drop, and full SMTP/IMAP draft/send pipeline integration remain open follow-up items.
