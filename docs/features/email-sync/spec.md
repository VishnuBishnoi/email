...
- The client **SHOULD** maintain one IDLE connection per active account, each monitoring that account's INBOX folder. This is subject to the platform-specific Resource Limits below — on iOS, accounts exceeding the cap fall back to periodic polling.
...
| 1.3.4 | 2026-02-16 | Core Team | PR review fixes (round 4). **P2**: FR-SYNC-12 — downgraded per-account IDLE from MUST to SHOULD with explicit deferral to Resource Limits subsection (was unqualified MUST that conflicted with iOS 5-cap in the same FR). |
