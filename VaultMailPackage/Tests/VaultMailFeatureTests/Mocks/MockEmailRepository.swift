...
final class MockEmailRepository: EmailRepositoryProtocol {
    ...
    var folders: [Folder] = []
    var emails: [Email] = []
    var threads: [VaultMailFeature.Thread] = []
    ...
    func getThreads(accountId: String) async throws -> [VaultMailFeature.Thread] {
        getThreadsCallCount += 1
        if let error = errorToThrow { throw error }
        return threads.filter { $0.accountId == accountId }
    }

    func getThread(id: String) async throws -> VaultMailFeature.Thread? {
        getThreadCallCount += 1
        if let error = errorToThrow { throw error }
        return threads.first { $0.id == id }
    }

    func saveThread(_ thread: VaultMailFeature.Thread) async throws {
    ...
    func getThreads(folderId: String, category: String?, cursor: Date?, limit: Int) async throws -> [VaultMailFeature.Thread] {
