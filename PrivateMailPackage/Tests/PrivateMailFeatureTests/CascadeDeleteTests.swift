...
struct CascadeDeleteTests {
    ...
    private func createFullHierarchy(in context: ModelContext) -> (Account, Folder, Email, Thread, EmailFolder, PrivateMailFeature.Attachment) {
        ...
        let attachment = PrivateMailFeature.Attachment(filename: "file.txt", mimeType: "text/plain", sizeBytes: 100)
    ...
    func emailDeleteCascades() throws {
        ...
        let attachments = try context.fetch(FetchDescriptor<PrivateMailFeature.Attachment>())
